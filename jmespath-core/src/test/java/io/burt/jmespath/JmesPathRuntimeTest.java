...
public abstract class JmesPathRuntimeTest<T> {
    ...
    protected T oddthings;

    protected Adapter<T> runtime() {
        return runtime;
    }
    ...
    protected T search(String query, T input) {
        return search(query, input, false).getValue();
    }

    protected Result<T> search(String query, T input, Boolean tracing) {
        Expression<T> expression = runtime().compile(query);
        return expression.search(input, tracing);
    }

    protected WrappedResult<T> searchWrapped(String query, T input, Boolean tracing) {
        Expression<T> expression = runtime().compile(query);
        return expression.searchWrapped(input, tracing);
    }

    protected List<String> paths(Result<T> results) {
        return results.getTraceNode().queriedPaths(runtime());
    }
    ...
    public void loadExamples() {
        ...
        oddthings = loadExample("/odd-things.json");
    }
    ...
    public void traceTest() {
        Result<T> result = search("Records[*].responseElements.instancesSet.items[*].instanceId", cloudtrail, true);

        assertThat(result.getValue(), is(parse("[[\"i-ebeaf9e2\"],[\"i-2e9faebe\"]]")));

        TraceNode<T> traceTree = result.getTraceNode();
        List<String> traces = paths(result);

        System.out.println(traceTree);
        System.out.println(traces);

        assertThat(traces.toString(), traces.size(), is(2));
        assertThat(traces.get(0), is("Records[0].responseElements.instancesSet.items[0].instanceId"));
        assertThat(traces.get(1), is("Records[1].responseElements.instancesSet.items[0].instanceId"));

    }

    @Test
    public void traceTest_oddArrays() {
        Result<T> result = search("oddArrays[*][*].two", oddthings, true);

        assertThat(result.getValue(), is(parse("[[\"value2A\"],[\"value2B\"],[\"value2C\"]]")));
        ...
        assertThat(traces.toString(), traces.size(), is(3));
        assertThat(traces.get(0), is("oddArrays[0][1].two"));
        assertThat(traces.get(1), is("oddArrays[1][1].two"));
        assertThat(traces.get(2), is("oddArrays[2][1].two"));
    }

    @Test
    public void traceTestWithFlattenEnding() {
        Result<T> result = search("Records[*].responseElements.instancesSet.items[*][].instanceId", cloudtrail, true);
        ...
        assertThat(traces.toString(), traces.size(), is(1));
        assertThat(traces.get(0), is("Records"));
    }

    @Test
    public void traceTestWithPipeEnding() {
        Result<T> result = search("Records[*].responseElements.instancesSet.items[*]|[]|[0].instanceId", cloudtrail, true);

        assertThat(result.getValue(), is(parse("\"i-ebeaf9e2\"")));
    ...
    @Test
    public void traceTestWithLongerCommonMergingPath() {
        Result<T> result = search("commonBase[?key == 'one'].values[*].item|[]", oddthings, true);

        assertThat(result.getValue(), is(parse("[\"Hello There\",\"2017-08-01\",1234]")));
        ...
        assertThat(traces.toString(), traces.size(), is(1));
        assertThat(traces.get(0), is("commonBase[0].values"));
    }

    @Test
    public void traceTestWithSkippedIndexesInSelectAndProjection() {
        Result<T> result = search("commonBase[?key == 'two'].values[*].item", oddthings, true);

        assertThat(result.getValue(), is(parse("[[\"null\"]]")));
        ...
        assertThat(traces.toString(), traces.size(), is(1));
        assertThat(traces.get(0), is("commonBase[1].values[1].item"));
    }

    @Test
    public void traceTestIgnoresNullBranchesAndContinues() {
        Result<T> result = search("Records[*].responseElements.instancesSet.items[?instanceId == 'i-ebeaf9e2'][].instanceId", cloudtrail, true);
        ...
        assertThat(traces.toString(), traces.size(), is(1));
        assertThat(traces.get(0), is("Records[0].responseElements.instancesSet.items[0]"));
    }

    @Test
    public void traceTestIgnoresNullBranches() {
        Result<T> result = search("Records[*].responseElements.instancesSet.items[?instanceId == 'i-ebeaf9e2']", cloudtrail, true);

        assertThat(result.getValue(), is(parse("[[{\"instanceId\":\"i-ebeaf9e2\",\"currentState\":{\"code\":0,\"name\":\"pending\"},\"previousState\":{\"code\": 80,\"name\":\"stopped\"}}],[]]")));
    ...
    @Test
    public void traceTestSelectionKeepsIndices() {
        Result<T> result = search("Records[?userAgent == 'ec2-api-tools 1.6.12.2'].userIdentity.userName", cloudtrail, true);

        assertThat(result.getValue(), is(parse("[\"Alice\",\"Bob\"]")));
        ...
        assertThat(traces.toString(), traces.size(), is(2));
        assertThat(traces.get(0), is("Records[0].userIdentity.userName"));
        assertThat(traces.get(1), is("Records[1].userIdentity.userName"));
    }


    @Test
    public void traceTestWithNoProjections() {
        Result<T> result = search("Records[0].responseElements.instancesSet.items[0].instanceId", cloudtrail, true);
    ...
    @Test
    public void traceTestAsJsonNormalizedPath() {
        WrappedResult<T> result = searchWrapped("Records[0].responseElements.instancesSet.items[0].instanceId", cloudtrail, true);

        assertThat(result.getValue(), is(parse("\"i-ebeaf9e2\"")));

        TraceNode<T> traceTree = result.getTraceRoot().getRoot();
        List<String> traces = result.getTraceRoot().getQueriedPaths();
        List<String> jsonPaths = result.getTraceRoot().getQueriedJsonPathCompatiblePaths();
        ...
        assertThat(jsonPaths.toString(), jsonPaths.size(), is(1));
        assertThat(jsonPaths.get(0), is("$['Records'][0]['responseElements']['instancesSet']['items'][0]['instanceId']"));
    }

    @Test
    public void traceTestRoot() {
        Result<T> result = search("@", cloudtrail, true);
